...
sys.path.append("D:\\code\\MDC-LDM\\iddm")
from torch import nn as nn
from torch import distributed as dist
from torch.utils.tensorboard import SummaryWriter
from torch.cuda.amp import autocast
from tqdm import tqdm
from config.setting import MASTER_ADDR, MASTER_PORT, EMA_BETA, LATENT_CHANNEL, IMAGE_CHANNEL, IMAGE_SCALE
...
from iddm.utils.dataset import get_value
...
class DMTrainer(Trainer):
    ...
    def before_train(self):
        ...
        if not self.conditional:
            ...
            # self.model = Network(img_size=64, patch_size=8, in_chans=8, out_chans=8,
            #  embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
            #  window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
            #  drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
            #  norm_layer=nn.LayerNorm, ape=True, patch_norm=True,
            #  use_checkpoint=False, fused_window_process=False).to(self.device)
        else:
    ...
    def train_in_iter(self):
        ...
        for i, (images, img_points_radiation) in enumerate(self.pbar):
            ...
            if self.latent:
                with torch.no_grad():
                    ...
                    labels = F.interpolate(img_points_radiation, size=self.image_size, mode='nearest')
                    labels = labels.repeat(1, self.latent_channels, 1, 1)
        ...
        if self.save_models:
            ...
            if not self.conditional:
                ...
                if self.vis:
                    ...
                    img_points_radiation = Points_Radiation(img, image_size=64, num_points=100, dis_ratio=0.1)
                    img_points_radiation = torch.from_numpy(img_points_radiation).float()
                    img_points_radiation = img_points_radiation.unsqueeze(0)
                    img_points_radiation = img_points_radiation.unsqueeze(0)
                    labels = img_points_radiation.repeat(1, self.latent_channels, 1, 1).to(self.device)
